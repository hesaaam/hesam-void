...
class LoadBalancerService extends ChangeNotifier {
  ...
  Future<void> setServers(List<VpnConfig> configs) async {
    _servers = configs.map((c) => ServerWithStats(
      config: c,
      ping: null,
      successRate: 100,
    ...
    try {
      ...
      if (_vpnService.isConnected && currentServer != null) {
        ...
        if (currentPing > _pingThreshold) {
          ...
          if (kDebugMode) {
            debugPrint('[LoadBalancer] Current ping ($currentPing) exceeds threshold ($_pingThreshold)');
          }
  ...
  Future<void> _switchToBetterServer() async {
    ...
    if (nextServer != null && nextServer.config.id != currentServer?.config.id) {
      if (kDebugMode) {
  ...
  Map<String, dynamic> getStats() {
    return {
      ...
      'avgPing': _servers.isNotEmpty
          ? _servers.where((s) => s.ping != null && s.ping! > 0)
              .map((s) => s.ping!)
              .fold(0, (a, b) => a + b) / 
